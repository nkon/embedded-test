...
ここの例では`printf()`のテストが書かれている。通常、`printf()`は標準出力、ここではUARTなどに出力するが、出力関数を差し替えることでバッファに出力し、バッファの内容を期待値と`strcmp()`することによって、合否判定を行っている。組込領域では、このように、なんらかの処理を行った結果がハードウエア出力になったり、ソフトウエア入力からなんらかの処理をすることが多く行われる。ハードウエアテストを行うためにも、ユニットテストを行うためにも、ハードウエアの操作とソフトウエアの操作は極力分離しなければならない。
